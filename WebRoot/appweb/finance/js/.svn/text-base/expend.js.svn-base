	...
	'->',
	{
			  ...
			  listeners:{
			  	'expand':function(){
			  		...
			  		loader.baseParams.pid="FT_002";//只加载支出节点
...
function reloadStore(path){
	if(path==null||path==""){
		ds.baseParams.typPath="/-1/FT_002";//支出
	}else{
		ds.baseParams.typPath=path;//支出
	}
	ds.load();
}
...
function addForm(){
	var form=new Ext.form.FormPanel({
		...
		labelAlign:'right',
		frame:true,
		id:'from-finance-expend-editFrom',
	...
	new Ext.Window({
		title:'编辑支出详细信息',
		id:'win-finance-expend-formGrid',
		width:450,
		modal:true,
		height:340,
		items:form,
		buttons:[
			{text:'保存',handler:saveFN,formBind:true},
			{text:'取消',handler:function(){
				Ext.getCmp("win-finance-expend-formGrid").destroy();
			}}
		]
	}).show();
}
function createTree(){
	...
	root:new Ext.tree.TreeNode({
		...
		text:'财务类型-->支出',
		children:[
			{text:'支出',id:'FT_002',expanded :true}
		]
	}),
	loader:new Ext.tree.TreeLoader({
			dataUrl :ACTION_TREE_URL,
			baseParams :{pid:'FT_002'},
		    listeners: {
...
function saveFN(){
	var bisicForm=Ext.getCmp("from-finance-expend-editFrom").getForm();
	var va=bisicForm.isValid();
	if(va===false)return;
	bisicForm.submit({
		url:ACTION_SAVE_URL ,
		scope :this,	
		...
		success:function(form ,action){
			...
			reloadStore();
			Ext.getCmp("win-finance-expend-formGrid").destroy();//销毁窗体
			if(Ext.getCmp("financetype-tree-select"))Ext.getCmp("financetype-tree-select").destroy();//销毁类型树
		},
